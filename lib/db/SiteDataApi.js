...
async function convertNotionToSiteData(
  ...
  // 站点域名属于部署身份，避免 Notion 模板配置污染 canonical、RSS 和 sitemap。
  NOTION_CONFIG.LINK = BLOG.LINK
  NOTION_CONFIG.CONTACT_GITHUB = BLOG.CONTACT_GITHUB
  if (process.env.NODE_ENV === 'development') {
    console.log(
  ...
  collectionData.forEach(element => {
    ...
    element.slug = normalizeOwnerLink(element.slug)
    element.href = normalizeOwnerLink(element.href)
  })
  ...
  if (allPages && allPages.length > 0) {
    allPages.forEach(p => {
      ...
      customNav.push({
        icon: p.icon || null,
        name: p.title || p.name || '',
        href: normalizeOwnerLink(p.href),
        target: p.target,
        show: true
      })
    })
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  const defaultTitle = BLOG.TITLE || NOTION_CONFIG?.TITLE || 'NotionNext BLOG'
  const defaultDescription =
    BLOG.DESCRIPTION ||
    NOTION_CONFIG?.DESCRIPTION ||
    '记录技术成长、工程实践与长期思考'
